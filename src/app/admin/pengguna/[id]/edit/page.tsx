...
export default function EditPenggunaPage() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setSubmitError("Gagal memuat data pengguna");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async () => {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus ${userData.name}? Tindakan ini tidak dapat dibatalkan.`)) {
      return;
    }
  ...
  // Extract team names from TEAMS array (objects with id, name, color)
  const teamNames = TEAMS.map(t => t.name).filter(t => t !== "Semua Tim");
      ...
      <div className="max-w-[800px] mx-auto p-8">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <span className="text-slate-900 font-medium">Edit Pengguna</span>
          </div>
        </div>
        ...
        <div className="mt-8 bg-white border border-red-200 rounded-xl p-6">
          ...
          <p className="text-sm text-slate-600 mb-4">
            Menghapus pengguna akan menghilangkan semua data terkait dan tidak dapat
            dikembalikan.
          </p>
          <button
            type="button"
